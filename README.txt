CEILING PRICE DATA COMPARE - FINAL PWA PROJECT (v3.7)
===============================================
...
---------------------
Reverted the v3.4 fixed-decimal behavior. MRP as per Ceiling Price and
MRP as per WPI now follow the Rounding dropdown selection (4 / 2 / 1 / 0
decimals) just like Existing MRP and Ceiling Price do. Proposed MRP is
back to its original logic too -- it also follows the Rounding dropdown
and uses normal rounding, not the truncate-to-2-decimal behavior added
in v3.4 (that truncate() helper was removed as it's no longer used).

Excel exports for these three columns also revert to exporting the raw
calculated value (same as Existing MRP and Ceiling Price already did),
rather than a rounded/truncated copy.

sw.js's cache name was bumped to v11 (from v10) so this update loads
cleanly through the auto-update mechanism.

What changed in v3.7
---------------------
The "Comparison Reference" panel and the "Comparison sequence:" note
(the two blocks explaining the matching rules and MRP formulas) are now
hidden behind a password prompt. Each shows a lock message with a
password field and Unlock button in its place until the correct
password is entered. Password: 1984

Notes on how this protection works:
- This is a fully static, client-side site with no server or login
  system, so this is a simple UI gate to keep the reference/formula
  details from casual viewers -- it is NOT real security. Anyone who
  views the page source (or your repo on GitHub) can find the password
  in plain text in index.html.
- The two sections unlock independently -- entering the password for one
  does not automatically unlock the other.
- Once unlocked, a section stays visible for the rest of that browser
  tab/session (uses sessionStorage) and re-locks automatically when the
  tab is closed. Each section also has a "Hide this section again" link
  to manually re-lock it without closing the tab.
- All other functionality (upload, compare, results table, exports) is
  completely unaffected and requires no password.

sw.js's cache name was bumped to v12 (from v11) so this update loads
cleanly through the auto-update mechanism.
